...
void CameraPoseVisualization::publish_by(ros::Publisher &pub, ros::Time &stamp )
{
    if(m_markers.size() == 0)
    {
        std::cout << "camera pose visualization quit!" << std::endl;
        return;
    }
    ...
    printf("publish %d camera poses!\n", markerArray_msg.markers.size());
}